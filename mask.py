...
def avg_skp(fitsfiles, n_amps, skp_img):
...
def plot(data):
    ...
    ax.set_ylabel("{} (ADU)".format(data.name))
    plt.show()
    return
...
def ROOThist(data, bin_sz, lam, norm):
    ...
    #n_bins=100/bin_sz
    hist = ROOT.TH1F("hist","hist", int(n_bins), 0, np.amax(data.data))
    ...
    hist.GetXaxis().SetTitle("{} (ADU)".format(data.name))
...
def make_mask(meds, MADs, MAD_std, MAD_mean, med_std, med_mean):
    ...
    MAD_mask = np.ma.masked_where(MADs.data>MAD_mean+MAD_std*1.75,MADs.data,copy=False)
...
def main():
    ...
    stacked = avg_skp(args.files, n_amps=2, skp_img=True)
    e_tot, median, MAD = projection_x(stacked[3200:-1,:], args.gain)
    plot(e_tot)
    plot(median)
    plot(MAD)
    MAD_std, MAD_mean = ROOThist(MAD, 0.1, 12, 3100)
    med_std, med_mean = ROOThist(median, 0.1, 12, 3100)
